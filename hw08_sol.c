...
enum parse_result parse_term_rest(struct stream *s) {
  struct token peeked = peek(s);
  if (peeked.type == TOK_BINOP && *peeked.start == '*') {
    next(s);
    return parse_term(s);
  }
  return PARSE_SUCCESS;
...
#ifdef INSTRUCTOR_FACTOR
enum parse_result parse_factor(struct stream *s) {
  struct token peeked = peek(s);
  if (peeked.type == TOK_LPAREN) {
    next(s);
    ...
    if (next(s).type != TOK_RPAREN)
      return PARSE_FAILURE;
    return PARSE_SUCCESS;
  } else if (peeked.type == TOK_LITERAL) {
    next(s);
    return PARSE_SUCCESS;
  }
  return PARSE_FAILURE;
